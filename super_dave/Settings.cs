...
public class Settings {
	...
	public void load(DDPlugin plugin) {
		...
		// Boat
		m_boat_walk_speed_boost = this.m_plugin.Config.Bind<float>("Boat", "Boat - Walk Speed Boost", 0f, "Speed boost applied to Dave when walking on the boat (float, default 0f [set to 0 to disable], Recommended: 2f).");

        // Farm
        m_farm_walk_multiplier = this.m_plugin.Config.Bind<float>("Farm", "Farm - Walk Multiplier", 0f, "Multiplier applied to Dave when walking/sprinting on the farm (float, default 0f [ < 1 == faster, < 1 == slower, set to 0 to disable],  Recommended: 3f)\n\t[NOTE: Setting this value too high (above ~5) can make it possible for Dave to walk fully off the screen and get stuck]." );

        // Fish Farm
        m_fish_farm_walk_multiplier = this.m_plugin.Config.Bind<float>("Fish Farm", "Fish Farm - Walk Multiplier", 0f, "Multiplier applied to Dave when walking/sprinting on the fish farm (float, default 0f [ < 1 == faster, < 1 == slower, set to 0 to disable], Recommended: 3f)." );
		...
		m_sushi_speed_boost = this.migrate_option<float>( "Sushi", "Sushi Speed Boost", "Sushi - Speed Boost", 0f, "Permanent speed boost when working in sushi bar (float, default 0f [set to 0 to disable], Recommended: 1.5f)." );
		...
		// Diving
		m_speed_boost = this.migrate_option<float>( "Diving", "Speed Boost", "Diving - Speed Boost", 0f, "Permanent speed boost when diving (float, default 0f [set to 0 to disable], Recommended: 1.2f)." );
        ...
        m_weightless_items = this.migrate_option<bool>("Diving", "Diving - Weightless Items (Infinite Carry Weight)", "Diving - Weightless Items (Infinite Carry Weight)", false, "Set to true to have reduce the weight of all items to 0, effectively giving infinite carry weight and inventory space.");
