...
public class Settings {
	...
	public static ConfigEntry<string> m_hotkey_Weapon_Down;
	...
	public void load(DDPlugin plugin) {
		...
		// Hotkeys
		m_hotkey_modifier = this.m_plugin.Config.Bind<string>( "Hotkeys", "Hotkey - Modifier", "LeftControl,RightControl", "Comma-separated list of Unity Keycodes used as the special modifier key (i.e. ctrl,alt,command) one of which is required to be down for hotkeys to work.\nSet to '' (blank string) to not require a special key (not recommended).\nSee this link for valid Unity KeyCode strings (https://docs.unity3d.com/ScriptReference/KeyCode.html)" );
		...
		m_hotkey_Weapon_Up = this.m_plugin.Config.Bind<string>( "Hotkeys", "Hotkey - Weapon Up", "KeypadPlus", "Increase Weapon Level.\nSee this link for valid Unity KeyCode strings (https://docs.unity3d.com/ScriptReference/KeyCode.html)" );
		m_hotkey_Weapon_Down = this.m_plugin.Config.Bind<string>( "Hotkeys", "Hotkey - Weapon Down", "KeypadMinus", "Decrease Weapon Level.\nSee this link for valid Unity KeyCode strings (https://docs.unity3d.com/ScriptReference/KeyCode.html)" );

		// Boat
		m_boat_walk_speed_boost = this.m_plugin.Config.Bind<float>("Boat", "Boat - Walk Speed Boost", 2f, "Speed boost applied to Dave when walking on the boat (float, default 0f [set to 0 to disable]).");

        // Farm
        m_farm_walk_multiplier = this.m_plugin.Config.Bind<float>("Farm", "Farm - Walk Multiplier", 3f, "Multiplier applied to Dave when walking/sprinting on the farm (float, default 0f [ < 1 == faster, < 1 == slower, set to 0 to disable]) [NOTE: Setting this value too high (above ~5) can make it possible for Dave to walk fully off the screen and get stuck].");

        // Fish Farm
        m_fish_farm_walk_multiplier = this.m_plugin.Config.Bind<float>("Fish Farm", "Fish Farm - Walk Multiplier", 3f, "Multiplier applied to Dave when walking/sprinting on the fish farm (float, default 0f [ < 1 == faster, < 1 == slower, set to 0 to disable]).");
		...
		m_sushi_speed_boost = this.migrate_option<float>( "Sushi", "Sushi Speed Boost", "Sushi - Speed Boost", 2f, "Permanent speed boost when working in sushi bar (float, default 0f [set to 0 to disable])." );
		...
		// Diving
		m_speed_boost = this.migrate_option<float>( "Diving", "Speed Boost", "Diving - Speed Boost", 1.2f, "Permanent speed boost when diving (float, default 0f [set to 0 to disable])." );
        ...
        m_auto_pickup_fish = this.m_plugin.Config.Bind<bool>("Diving", "Diving - Auto-pickup: Fish", true, "Set to true to enable auto pickup of sleeping/netted fish (NOTE: This will disable the Large Pickups option to prevent drone glitches; just be sure to enable Infinite Drones when using this one).");
        ...
        m_auto_pickup_items = this.m_plugin.Config.Bind<bool>("Diving", "Diving - Auto-pickup: Items", true, "Set to true to enable auto pickup of items (wood, pots, etc).");
        ...
        m_harpoon_head_type = this.m_plugin.Config.Bind<string>("Diving", "Diving - Harpoon Head Type", "Sleep", "Harpoon head type (one of: Normal, Electric, Poison, Chain, Sleep, Paralysis, Strong, Fire, Ice) [case sensitive, set to blank to disable].");
        m_harpoon_head_level = this.m_plugin.Config.Bind<int>("Diving", "Diving - Harpoon Head Level", 3, "Harpoon head level [ignored if Harpoon Head Type is not set] (int, 1 (weakest) - 5 (strongest)).");			
        m_infinite_bullets = this.migrate_option<bool>("Diving", "Infinite Bullets", "Diving - Infinite Bullets", true, "Set to true to have infinite bullets when diving.");
        ...
        m_infinite_drones = this.m_plugin.Config.Bind<bool>("Diving", "Diving - Infinite Drones", true, "Set to true to enable infinite salvage drones.");
        ...
        m_weightless_items = this.migrate_option<bool>("Diving", "Weightless Items (Infinite Carry Weight)", "Diving - Weightless Items (Infinite Carry Weight)", true, "Set to true to have reduce the weight of all items to 0, effectively giving infinite carry weight and inventory space.");
